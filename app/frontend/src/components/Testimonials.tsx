...
const Testimonials = () => {
  ...
  return (
    <section id="temoignages" className="py-24 bg-[#0A0E27] relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-full">
        <motion.div
          className="absolute top-20 right-20 w-72 h-72 bg-[#0066FF] rounded-full mix-blend-screen filter blur-3xl opacity-20"
          animate={{ x: [0, 40, 0], y: [0, -30, 0], scale: [1, 1.2, 1] }}
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute bottom-20 left-20 w-72 h-72 bg-[#00D4FF] rounded-full mix-blend-screen filter blur-3xl opacity-10"
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <AnimatedSection animation="fade-up" className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-block px-4 py-1 rounded-full bg-white/10 text-[#00D4FF] text-sm font-medium mb-4 backdrop-blur-sm border border-white/10">
            {badge}
          </div>
          ...
          {testimonials.map((testimonial, index) => (
            <StaggerItem key={index} animation="fade-up">
              <motion.div
                ...
                className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 hover:bg-white/10 hover:border-[#0066FF]/50 transition-colors duration-300 relative h-full"
              >
